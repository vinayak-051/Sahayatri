...
import { Plus, Trash2, Pencil, MapPin, ArrowLeft, Image as ImageIcon } from "lucide-react";
    ...
    <div className="min-h-screen gradient-sky pb-24 text-foreground">
      ...
      {showAddForm ? (
        <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="px-6 py-4">
          <form onSubmit={handleSubmit} className="glass rounded-2xl p-6 shadow-card space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <input required type="text" placeholder="Cost of trip (e.g. ₹5000 per person)" className="w-full glass rounded-xl px-4 py-3 text-sm" value={formData.cost} onChange={(e) => setFormData({ ...formData, cost: e.target.value })} />
            ...
            <div className="space-y-1">
              <p className="text-xs text-muted-foreground">Tap on the map to set this location's position</p>
              <LocationPicker
                key={editingId ?? "new"}
        ...
        <div className="px-6 space-y-4">
          {locations.map((loc) => (
            <motion.div key={loc.id} initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="glass rounded-2xl p-4 shadow-card flex gap-4">
